...
class SignalModel(jft.Model):
    def __init__(self, diffuse, points=None):
        ...
        self.points = points
        super().__init__(
            init=diffuse.init | points.init if points is not None else diffuse.init
        )

    def __call__(self, x):
        return (
            self.diffuse(x) + self.points(x)
...
def data(
    *,
    ...
    noise_truth = ((noise_cov(jft.ones_like(signal.target))) ** 0.5) * jft.random_like(
        k_n, signal.target
    )
...
def transition(
    *,
    ...
    def tr(key, samples, it):
        ...
        for x, y in zip(coos[0], coos[1], strict=False):
            mask[
                max(0, x - margin) : min(rec.shape[0], x + margin + 1),
                max(0, y - margin) : min(rec.shape[1], y + margin + 1),
            ] = 0
        plot_arr(mask, name="mask", odir=odir)

        # mask the diffuse part of the new signal model at the coordinates of the point sources
        sig_df = jft.Model(
            lambda x: sig_new.diffuse(x) * mask, init=sig_new.diffuse.init
        )

        # add some noise to the reconstruction
        k_n, k_o, k_i = jax.random.split(key, 3)
        noise_cov = lambda x: noise_var**2 * x
        noise = ((noise_cov(jft.ones_like(sig_df.target))) ** 0.5) * jft.random_like(
            k_n, sig_df.target
        )
        data = rec + noise
...
sig_dct = {
    sec: cfg.instantiate_sec(sec) for sec in cfg.sections if sec.startswith("sig")
}
