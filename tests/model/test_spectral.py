...
# Uses the shared ``small_grid`` fixture (8×8) from tests/conftest.py.
...
class TestMultiFrequencyModelSingleFreq:
    ...
    def test_output_shape(self, small_grid):
        i0 = normal_model(prefix="i0 ", shape=small_grid.shape, mean=0.0, std=1.0)
    ...
    def test_no_nonlinearity(self, small_grid):
        i0 = normal_model(prefix="i0 ", shape=small_grid.shape, mean=0.0, std=1.0)
...
class TestSpectralPriorModelSingleFreq:
    ...
    def test_single_freq_build(self, small_grid):
    ...
    def test_multi_freq_requires_alpha(self, small_grid):
    ...
    def test_multi_freq_with_alpha(self, small_grid):
        model = spectral_prior_model(
            prefix="sp ",
            grid=small_grid,
