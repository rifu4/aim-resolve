...
from .util import clean_dict, merge_dicts, pop_val, split_its, update_it
...
class SetupKLConfig:
    ...
    def add_it(self, fix_keys=None, del_comp=True, it=None):
        ...
        upd_keys = [
            k
            for k in upd_keys
            if k not in fix_keys and any(x in k for x in ["opt", "lh", "data", "sky"])
        ]
