    ...
    with jax.default_device(device):
        ...
        match (in_array.ndim, in_copies, out_copies):
            ...
            case (2, 1, 1):
                ...
                res = map_function(in_array, out_array, in_start, out_start, in_shape, zoom)
            # (x, y) -> (n_out, x', y')
            case (2, 1, n_out):
                ...
                res = fori_loop(0, n_out, loop_over_n_out, out_array)

            # (n/f, x, y) -> (n/f, x', y')
            case (3, 1, 1):
                n = in_array.shape[0]
                out_array = jnp.zeros((n,) + out_shape, dtype=in_array.dtype)
                res = vmap(map_function, in_axes=(0, 0, None, None, None, None))(in_array, out_array, in_start, out_start, in_shape, zoom)
            # (n_in, x, y) -> (x', y')
            case (3, n_in, 1):
                out_array = jnp.zeros(out_shape, dtype=in_array.dtype)
                loop_over_n_in = lambda i,o : map_function(in_array[i], o, in_start[i], out_start[i], in_shape, zoom)
                res = fori_loop(0, n_in, loop_over_n_in, out_array)
            ...
            case (4, 1, 1):
                ...
                res = fori_loop(0, n, loop_over_n, out_array)
            # (n_in, f, x, y) -> (f, x', y')
            case (4, n_in, 1):
                f = in_array.shape[1]
