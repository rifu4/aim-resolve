"""Single-frequency talk plots for ESO reconstructions."""
...
# %%
eso_dir = (
    "/Users/rf/Development/packages/aim-resolve/steering/runs/fast_vi_1f_1024_1z_b"
)
...
plot_arrays(
    ...
    callback=callback,
    grid_kwargs=dict(
        hspace=-0.5, wspace=-0.3, width_ratios=[1, 1, 1], height_ratios=[1]
    ),
    **plot_dict,
)
...
# %%
comp_mean_sf = [
    samples_sf[sky_it].mean(c) * CONV_FACTOR
    for c in [skies_sf[sky_it].objects[0], skies_sf[sky_it].objects[1]]
]
...
for m in sky_pt_sf.models:
    if m not in [
        skies_sf[sky_it].background,
        skies_sf[sky_it].objects[0],
        skies_sf[sky_it].objects[1],
    ]:
        pt_models.append(m)
