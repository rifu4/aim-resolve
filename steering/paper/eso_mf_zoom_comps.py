...
for t, a in zip(tiles_ref_mf, tiles_alpha):
    ...
    contours = {'array': t, 'colors': 'white', 'levels': [1e-2, 1e-1], 'linewidths': 0.5}
...
aim.plot_arrays(
    ...
    **plot_dict | dict(vmin=tiles_vmin, vmax=tiles_vmax, norm=tiles_norm, cmap=tiles_cmap),
)
...
aim.plot_arrays(
    ...
    **plot_dict | dict(vmin=sorted_tiles_vmin, vmax=sorted_tiles_vmax, norm=sorted_tiles_norm, cmap=sorted_tiles_cmap),
)
...
sorted_groups_ref = [sorted_tiles_ref[i:i + 8] for i in range(0, len(sorted_tiles_ref), 8)]
sorted_groups_alpha = [sorted_tiles_alpha[i:i + 8] for i in range(0, len(sorted_tiles_alpha), 8)]
...
for row_idx in range(8):
    for group_idx, (group_ref, group_alpha, group_peak) in enumerate(zip(sorted_groups_ref, sorted_groups_alpha, sorted_groups_peak)):
        t = group_ref[row_idx]
...
aim.plot_arrays(
    ...
    **plot_dict | dict(vmin=transposed_tiles_vmin, vmax=transposed_tiles_vmax, norm=transposed_tiles_norm, cmap=transposed_tiles_cmap),
)
# aim.plot_arrays(
