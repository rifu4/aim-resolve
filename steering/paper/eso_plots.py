...
# %%
eso_dir = (
    "/scratch/users/rfuchs/packages/aim-resolve/steering/runs/fast_vi_1f_1024_2z_a"
)
eso_it = 3
...
optim_cfg = OptimizeKLConfig.from_file(
    (eso_dir + "/files/base.yml", eso_yml), get_builders, "total"
)

sky_lst = [
    sec for sec in optim_cfg.sections if all(k in sec for k in ["sky", f".{eso_it}"])
]
...
grd = skies_c[0].grid
plots2 = [
    map_signal(grd, grd.update(space=grd.spc // 2))(val)
    for pair in zip(plot2_means, plot2_stds)
    for val in pair
]
...
plot_arrays(
    ...
    cbar=True,
    cbar_kwargs=[
        dict(loc="bottom", size="2.5%", pad="2.5%", label="mJy/arcsec$^2$"),
        dict(loc="bottom", size="2.5%", pad="2.5%", label="relative uncertainty"),
    ],
    figsize=(5, 5.5),
    cmap=cmap,
    # callback=callback,
...
plot_arrays(
    ...
    dpi=300,
    grid_kwargs=dict(
        hspace=-0.65, wspace=-0.5, width_ratios=[1, 1], height_ratios=[1, 0.5]
    ),
)
...
plot_arrays(
    ...
    cmap=cmap,
    grid_kwargs=dict(
        hspace=-0.57, wspace=-0.66, width_ratios=[1, 1], height_ratios=[1, 1, 1, 1]
    ),
    callback=callback,
)
...
plot_arrays(
    ...
    cmap=cmap,
    grid_kwargs=dict(
        hspace=-0.61, wspace=-0.42, width_ratios=[1, 1, 1, 1], height_ratios=[1, 1]
    ),
    callback=callback,
)
