    ...
    for idx, name in enumerate(regions_config):
        ...
        for shape_idx, (cen, ext) in enumerate(zip(region['center'], region['extend'])):
            ...
            if len(ext) == 1:  # Circle
                ...
                ymin = c_y - f_y / 2
                ymax = c_y + f_y / 2
                geom_mask = ((x_coords >= xmin) & (x_coords < xmax) &
            ...
            if 'value' in region and brightness is not None:
                if shape_idx < len(region['value']):
                    ...
                    value_mask = (brightness_values >= v_min) & (brightness_values <= v_max)
...
regions_config = aim.yaml_load('/scratch/users/rfuchs/packages/aim-resolve/steering/paper/regions.yml')
min_scatter = 5e-3
...
for idx, (c_ref, c_alpha) in enumerate(zip(comp_ref_mf[:1], comp_alpha[:1]), start=1):
    ...
    ax.contour(c_ref.T, colors='white', levels=[1e-2, 1e-1], linewidths=0.5, origin='lower')
    ...
    ax.legend(handles=legend_elements, loc='lower center', fontsize=7, ncol=3,
...
for idx, (c_ref, c_alpha) in enumerate(zip(comp_ref_mf[:1], comp_alpha[:1]), start=1):
    ...
    # Assign regions to pixels
    region_id, region_colors = assign_regions_to_pixels(mask, regions_config, brightness=c_ref)
    ...
    for region_idx, (name, region) in enumerate(regions_config.items()):
        ...
        if np.any(region_mask):
            plt.scatter(x[region_mask], y[region_mask], s=2, alpha=0.5,
                       edgecolors='none', color=region['color'], 
                       label=name)
    
    # Plot unassigned points in grey
    unassigned_mask = regions == -1
    if np.any(unassigned_mask):
        plt.scatter(x[unassigned_mask], y[unassigned_mask], s=2, alpha=0.5,
                   edgecolors='none', color='grey', label='unassigned')
