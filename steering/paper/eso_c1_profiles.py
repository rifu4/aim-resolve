...
# The per-column search is confined to a corridor of half-height RIDGE_Y_WINDOW
# (fraction of the image height) around the hand-drawn LINK_ANCHORS polyline, so
# the ridge follows the intended sub-part even if the galaxy is brighter
# elsewhere (set to None to search the full column). RIDGE_SMOOTH gaussian-
# smooths the traced y(x) [pixels; 0 disables] so the stripes don't jitter.
RIDGE_METHOD = "max"
RIDGE_SMOOTH = 5.0
RIDGE_Y_WINDOW = 0.001
...
# inside a corridor around the LINK_ANCHORS polyline (RIDGE_Y_WINDOW).
link_anchors = ridge_anchors(
    flux_c, LINK_ANCHORS, method=RIDGE_METHOD, smooth=RIDGE_SMOOTH, y_window_frac=RIDGE_Y_WINDOW,
)
...
# The distance axis unit is independent of the surface-brightness unit (which
# stays mJy/arcsec^2).
DIST_UNIT = "arcsec"  # "arcsec" or "arcmin" for the distance axis
...
def plot_link_profiles(name, brightness_std=False, curvature=False, ref_only=False):
    ...
    ax_img.imshow(
        ...
        norm="log", vmin=5e-3, vmax=float(np.nanmax(flux_c)), aspect="auto",
    )
    ...
    ax_i.set_xlabel(f"distance along CST1 {_DIST_UNIT_LABEL}  (left → right)")
...
plot_link_profiles("c1_profiles.png", brightness_std=True)
...
# ---------------------------------------------------------------------------
plot_link_profiles("c1_profiles_curvature.png", brightness_std=True, curvature=True)
