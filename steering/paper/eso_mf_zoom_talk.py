...
aim.plot_arrays(
    ...
    rows=2,
    grid_kwargs=dict(
        hspace=-0, wspace=-0, width_ratios=[1, 1, 1], height_ratios=[1, 1]
    ),
    # callback=callback,
    **plot_dict,
)
...
for idx, (c_ref, c_alpha) in enumerate(zip(comp_ref_mf, comp_alpha), start=1):
    ...
    for i in range(3):
        for j in range(3):
            grid_mask = (
                (y_coords >= y_split[i])
                & (y_coords < y_split[i + 1])
                & (x_coords >= x_split[j])
                & (x_coords < x_split[j + 1])
            )
...
for idx, (c_ref, c_alpha) in enumerate(zip(comp_ref_mf, comp_alpha), start=1):
    ...
    legend_elements = [
        Patch(facecolor=colors_9_rgb[i], label=region_names[i]) for i in range(9)
    ]
    ax.legend(
