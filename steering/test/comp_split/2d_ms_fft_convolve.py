...
#%%
response_high = fft_convolve_2d(signal_dict['sum']['val'], kernel, signal_dict['sum']['spc'])
response_low = fft_convolve_2d(signal_dict['sum']['low'], downsample(kernel, signal_dict['fac']), signal_dict['bg']['spc'])
...
plot_arrays([np.abs(err) for err in rsp_err], dpi=300, rows=1, norm='log', vmin=2e-5, vmax=1e-2)
...
def multi_scale_kernel(signal_dict, kernel):
    ...
    for key, sig in filter(lambda x: 't' in x[0], signal_dict.items()):
        ...
        ker_high = map_array(kernel, grid.update(factor=2*fac), sig_grid.update(center=(0,0), factor=2*fac))
        ...
        if key == 't0':
            plot_arrays([rsp_slc, rsp_low, rsp_high, rsp_add], dpi=300, rows=1, norm='log', vmin=1e-5, vmax=1e-2)
    ...
    for key, sig in filter(lambda x: 'ps' in x[0], signal_dict.items()):
        ...
        for i in range(ps_grid.n_copies):
            pi_grid = PointGrid(coordinates=(ps_grid.coordinates[i], ), factor=ps_grid.factor, n_copies=1)
            sig_grid = SignalGrid(space=(ker_size, ker_size), center=tuple(ps_cen[i].tolist()), factor=ps_grid.factor, distances=grid.distances)
            sig_grid_low = sig_grid.update(factor=1)
...
plot_arrays([response_high, response_low, response_ms], dpi=300, rows=1, vmin=1e-5, vmax=1e-1)
plot_arrays([response_high, response_low, response_ms], dpi=300, rows=1, norm='log', vmin=1e-5, vmax=1e-1)
...
plot_arrays([np.abs(err) for err in response_errors], dpi=300, rows=1, norm='log', vmin=1e-5, vmax=1e-2)
plot_arrays([np.abs(err/response_high) for err in response_errors], dpi=300, rows=1, norm='log', vmin=1e-4, vmax=1e4)
...
mcm = costs(1024) + (costs(256*4) + costs(256*2)) * 2 + (costs(64*4) + costs(64*2)) * 60
