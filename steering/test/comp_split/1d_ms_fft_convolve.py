...
def multi_scale_kernel(signal_dict, kernel, space):
    ...
    response_low = fft_convolve_1d(
        signal_dict["sum"]["low"], kernel_low, space * (1 / fac)
    )
    ...
    for key, sig in filter(lambda x: "t" in x[0], signal_dict.items()):
        ...
        ker_val = kernel[
            kernel.size // 2 - sig_val.size : kernel.size // 2 + sig_val.size
        ]
        rsp_val = fft_convolve_1d(
            sig_val, ker_val, space.transform(shape=sig_val.shape)
        )

        sig_low = sig["low"][sig["slc_low"]]
        ker_low = downsample(ker_val, fac)
        rsp_low = fft_convolve_1d(
            sig_low, ker_low, space.transform(shape=sig_val.shape) * (1 / fac)
        )
    ...
    for key, sig in filter(lambda x: "ps" in x[0], signal_dict.items()):
        ...
        ker_val = kernel[
            kernel.size // 2 - sig["ker_pix"] // 2 : kernel.size // 2
            + sig["ker_pix"] // 2
        ]
...
kernel_low = downsample(kernel, signal_dict["fac"])
response_low = fft_convolve_1d(
    signal_dict["sum"]["low"], kernel_low, space_2d_512 * (1 / signal_dict["fac"])
)
...
plot_1d(
    response_high, response_low, offset=10, figsize=(20, 3), title="response low res"
)
plot_1d(
    response_high,
    response_high,
