...
response_dict = {}
for z in zoom:
    response_dict[z] = upsample(
        convolve(signal_dict[z], kernel_dict[z], mode="same", method="fft") * z, z
    )

pdict = {"offset": 6, "figsize": (20, 3)}
plot_1d(
    [upsample(s, z) for z, s in signal_dict.items()],
    signal_dict.keys(),
    norm="log",
    title="signals",
    **pdict,
...
plot_1d(
    [upsample(k, z) for z, k in kernel_dict.items()],
    kernel_dict.keys(),
    norm="linear",
    title="kernels",
    **pdict,
...
plot_1d(
    [upsample(k, z) for z, k in kernel_split.items()],
    kernel_split.keys(),
    norm="linear",
    title="kernel split",
    **pdict,
...
def multi_scale_kernel(signal_dict, kernel_split):
    ...
    for z, k in kernel_split.items():
        response_split[z] = upsample(
            convolve(signal_dict[z], k, mode="same", method="fft") * z, z
        )
...
plot_1d(
    rsp_val, rsp_key, norm="linear", title="responses", **pdict, vmin=-3e4, vmax=3e4
)


plot_1d(
    [rv - response_dict[1] for rv in rsp_val],
    [f"{rk} - 1" for rk in rsp_key],
    norm="linear",
    title="response split error",
    **pdict,
)
plot_1d(
    [rv - response_dict[1] for rv in rsp_val[:2] + rsp_val[3:]],
    [f"{rk} - 1" for rk in rsp_key[:2] + rsp_key[3:]],
    norm="linear",
    title="response split error",
    **pdict,
    vmin=-3e3,
...
plot_1d(
    (response_sum - response_dict[1]) / response_dict[1],
    norm="linear",
    title="rel. response split error",
    **pdict,
    vmin=-1,
    vmax=1,
)
