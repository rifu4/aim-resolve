...
def plot_pspec(
    ...
    if isinstance(mdl, TileModel):
        ...
        for i, s in enumerate(smpls):
            if i == 0:
                ax.loglog(
                    x, mdl.pspec(s), label="reconstructed samples", c="gray", lw=0.25
                )
            else:
...
optim_cfg = OptimizeKLConfig.from_file(
    (exp_dir + "/files/base.yml", exp_yml), get_builders, "total"
)
optim_cfg.sections["data.0"]["fname"] = (
    "/Users/rf/Development/packages/aim-resolve/steering/"
    + optim_cfg.sections["data.0"]["fname"]
)
...
with open(exp_dir + "/opt/0_rec/last.pkl", "rb") as f:
...
plot_pspec(
    ...
    md,
    scm_samples,
    "M0 - background power spectrum: $P_{s_b}(|k|)$",
    ylabel=True,
    xlabel=True,
    ylim=ylim,
)
...
plot_pspec(
    ...
    md,
    mcm_samples,
    "MC - background power spectrum: $P_{s_b}(|k|)$",
    ylim=ylim,
)
...
plot_pspec(
    axs[-1], md, mcm_samples, "MC - tile 1 power spectrum: $P_{s_{t1}}(|k|)$", ylim=ylim
)
...
plot_pspec(
    ...
    md,
    mcm_samples,
    "MC - tile 2 power spectrum: $P_{s_{t2}}(|k|)$",
    ylabel=True,
    xlabel=True,
    ylim=ylim,
)
...
plot_pspec(
    ...
    md,
    mcm_samples,
    "MC - tile 3 power spectrum: $P_{s_{t3}}(|k|)$",
    xlabel=True,
    ylim=ylim,
)
